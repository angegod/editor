...
function ParView(){
    ...
    const ParContent=useRef({});//文章內容 其實可以不用同步
    ...
    const [isModify,setIsModify]=useState(false);//編輯與刪除按鈕是否有權限
    ...
    useEffect(()=>{
        ...
        axios.post("http://192.168.0.64:5000/par/search", search).then((response) => {
            if(response.data.length===0){
                ...
                ParContent.current=response.data[0];
    ...
    function ShowContent(){
        ...
        const htmldoc=parser.parseFromString(ParContent.current.content,'text/html');
        const searchP=htmldoc.querySelectorAll('p');
        ...
        for(var i=0;i<searchP.length;i++){
            ...
            if(i===pIndex)
                list.push(<>
                    <div className={`${(i===0)?'':'mt-5'} ${(i===searchP.length)?'':'mb-5'}`}><ReactTyped strings={[searchP[i].outerHTML]} typeSpeed={50} /></div>
                </>)
            else if(i<pIndex)
                list.push(<>
                    <div className={`${(i===0)?'':'mt-5'} ${(i===searchP.length)?'':'mb-5'}`} dangerouslySetInnerHTML={{__html:searchP[i].outerHTML}}></div>
                </>)
        ...
        <div className='ParView'>
            <div className='title mb-3'>
                <div className='flex flex-row justify-between'>
                    <span className='text-2xl font-bold text-amber-400 animate-bounce'>{ParContent.current.title}</span>
                ...
                <div className='ParDetails'>
                    <span>{ParContent.current.createDate}</span>
                    <span>作者:{ParContent.current.author}</span>
                </div>
            </div>
            <hr/>
            <div className='content mt-3' id="content" onClick={()=>setpIndex((count)=>count+=1)}>
                <ShowContent />
            </div>
